...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    Matrix61c* arg1 = (Matrix61c*) args;
    if (!PyObject_TypeCheck(arg1, &Matrix61cType)) {
            PyErr_SetString(PyExc_TypeError, "Argument must of type numc.Matrix!");
            return NULL;
        }
    if (self == NULL){
    ...
    int rows2 = arg1->mat->rows;
    int cols2 = arg1->mat->cols;
    
    if ((rows1 != rows2) | (cols1 != cols2) | (rows1 < 1) | (cols1 < 1)){
        PyErr_SetString(PyExc_TypeError, "Invalid arguments");
        return NULL;
    }
    matrix *new_mat;
    int alloc_failed = allocate_matrix(&new_mat, rows1, cols1);
    if (alloc_failed){
        PyErr_SetString(PyExc_RuntimeError, "Allocate failed");
        return NULL;
    }
    int check = add_matrix(new_mat, self->mat, arg1->mat);
    if (check == -1){
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
   Matrix61c* arg1 = (Matrix61c*) args;
    ...
    int check = sub_matrix(new_mat, self->mat, arg1->mat);
    if (check == -1){
         PyErr_SetString(PyExc_RuntimeError, "Failed to add");
         return NULL;
    }
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    int rows1 = self->mat->rows;
    int cols1 = self->mat->cols;
    if ((cols1 != rows2) | (rows1 < 1) | (cols1 < 1) | (rows2 < 1) | (cols2 < 1))     {
        PyErr_SetString(PyExc_TypeError, "Dimensions don't match");
        return NULL;
    }
    matrix *new_mat;
    int alloc_failed = allocate_matrix(&new_mat, rows1, cols2);
    if (alloc_failed){
        PyErr_SetString(PyExc_RuntimeError, "Allocate failed");
        return NULL;
    }
    int check = mul_matrix(new_mat, self->mat, arg1->mat);
    if (check == -1){
         PyErr_SetString(PyExc_RuntimeError, "Failed to mul");
         return NULL;
    }
    Matrix61c* rv = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    rv->mat = new_mat;
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    ...
    int a = neg_matrix(new_mat, self->mat);
    if (a == -1){
        PyErr_SetString(PyExc_TypeError, "Invalid arguments");
        return NULL;
    }
...
static PyObject *Matrix61c_abs(Matrix61c *self) {
 if (self == NULL)
    ...
    int a = abs_matrix(new_mat, self->mat);
    if (a == -1){
        PyErr_SetString(PyExc_TypeError, "Invalid arguments");
        return NULL;
    }
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    if (!PyLong_Check(pow)) {
        PyErr_SetString(PyExc_TypeError, "Power not valid");
        return NULL;
    }
    int power = PyLong_AsLong(pow);
    if (power < 0)
    {
        PyErr_SetString(PyExc_TypeError, "Power must be positive int");
    }
    if (self == NULL)
    {
        PyErr_SetString(PyExc_TypeError, "Invalid arguments");
    ...
    int a = pow_matrix(new_mat, self->mat,power);
    if (a == -1){
        PyErr_SetString(PyExc_TypeError, "Invalid arguments");
        return NULL;
    }
...
static PyNumberMethods Matrix61c_as_number =  {
     ...
     .nb_inplace_add = (binaryfunc) Matrix61c_add,
     .nb_subtract =  (binaryfunc) Matrix61c_sub,
     .nb_inplace_subtract =  (binaryfunc) Matrix61c_sub,
     .nb_multiply = (binaryfunc) Matrix61c_multiply,
     .nb_inplace_multiply = (binaryfunc) Matrix61c_multiply,
     .nb_power = (ternaryfunc) Matrix61c_pow,
     .nb_inplace_power = (ternaryfunc) Matrix61c_pow,
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    int *row_indexpointer = NULL;
    int *col_indexpointer = NULL;
    double *valpointer = NULL;
    if(!(PyArg_UnpackTuple(args, "args", 3, 3,&row_indexpointer,&col_indexpointer,&valpointer))){
        PyErr_SetString(PyExc_TypeError, "toomany argue");
        return NULL;
    }

    if (!(row_indexpointer && col_indexpointer && valpointer && PyLong_Check(row_indexpointer) && PyLong_Check(col_indexpointer) &&  
    (PyFloat_Check(valpointer)) && (!PyBool_Check(row_indexpointer)) && (!PyBool_Check(col_indexpointer)) && (!PyBool_Check(col_indexpointer)) &&
    (!PyBool_Check(valpointer)))){
        PyErr_SetString(PyExc_TypeError, "Invalid arguments");
        return NULL;
    }
     double val;

    if (PyLong_Check(valpointer))
    {
       val = PyLong_AsLong(valpointer);
    }else
    {
        val = PyFloat_AsDouble(valpointer);
    }
    
    int row_index = PyLong_AsLong(row_indexpointer);
    int col_index =  PyLong_AsLong(col_indexpointer);
   
    if (self == NULL){
        PyErr_SetString(PyExc_TypeError, "self null");
        return NULL;
    }
    int row1 = self->mat->rows;
    int col1 = self->mat->cols;
    if ((row1 < 1) | (col1 < 1)){
        PyErr_SetString(PyExc_TypeError, "dim of original wrong");
        return NULL;
    }
    if ((row_index < 0) | (col_index < 0)){
        PyErr_SetString(PyExc_IndexError, "index smallers than 0");
        return NULL;
    }
    if ((row_index >= row1) |  (col_index >= col1))
    {
        PyErr_SetString(PyExc_IndexError, "index bigger than mat col or row");
        return NULL;
    }
    set(self->mat,row_index,col_index,val);    
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
    PyObject *rowpoint = NULL;
    PyObject *colpoint = NULL;
    if (!(PyArg_UnpackTuple(args, "args", 2, 2, &rowpoint, &colpoint))) {
         PyErr_SetString(PyExc_TypeError, "Invalid unpacking");
         return NULL;      
    }
    if (!(rowpoint && colpoint && PyLong_Check(rowpoint) && PyLong_Check(colpoint) && (!PyBool_Check(rowpoint)) && (!PyBool_Check(colpoint)))){
        PyErr_SetString(PyExc_TypeError, "Invalid arguments");
        return NULL;
    }
    int row_index = PyLong_AsLong(rowpoint);
    int col_index = PyLong_AsLong(colpoint);
    
    if ((row_index < 0) | (col_index < 0)){
        PyErr_SetString(PyExc_IndexError, "toolow");
        return NULL;
    }
    int row1 = self->mat->rows;
    int col1 = self->mat->cols;
    if ((row_index >= row1) |  (col_index >= col1))
    {
        PyErr_SetString(PyExc_IndexError, "exed");
        return NULL;
    }
    ...
    if ((row1 < 1) | (col1 < 1)){
        PyErr_SetString(PyExc_TypeError, "col of self less than1");
        return NULL;
    }
    
    double result = get(self->mat,row_index,col_index);    
...
static PyMethodDef Matrix61c_methods[] = {
    {"get", Matrix61c_get_value,METH_VARARGS,"getvalue"},
    {"set", Matrix61c_set_value,METH_VARARGS,"getvalue"},
    {NULL, NULL, 0, NULL}
};
